...
def _missing(
    symbol: str, reason: str, identity: str | None = None
) -> HistoricalSnapshotRow:
    return HistoricalSnapshotRow(
        symbol,
        None,
        None,
        None,
...
def discover_historical_universe(
    ...
    *,
    ...
    decision = (
        decision_time
        if isinstance(decision_time, DiscoveryDecisionTime)
        else DiscoveryDecisionTime(decision_time)
    )
    normalized = sorted(
    ...
    for symbol in normalized:
        ...
        try:
            ...
            # MarketDataStore uses ValueError for recognized-but-invalid source
            # archives during canonical validation. Isolate that failure to this
            # symbol rather than aborting the whole historical universe scan.
            sources.append(HistoricalSource(symbol, request, signature))
            rows.append(_missing(symbol, "source_validation_failed", identity))
            continue
        ...
        candidates["_period_start"] = pd.to_datetime(
            candidates["period_start"], utc=True, errors="coerce"
        )
        candidates["_period_end"] = pd.to_datetime(
            candidates["period_end"], utc=True, errors="coerce"
        )
        candidates["_available_at"] = pd.to_datetime(
            candidates["available_at"], utc=True, errors="coerce"
        )
        ...
        rows.append(
            HistoricalSnapshotRow(
                symbol,
                _utc(selected["_period_start"]),
                _utc(selected["_period_end"]),
                _utc(selected["_available_at"]),
                values["open"],
                values["high"],
                values["low"],
                values["close"],
    ...
    ranked = sorted(
        ...
        key=lambda row: (
            -row.range_percent,
            -row.absolute_price_change_percent,
            -row.quote_volume,
            row.symbol,
        ),
    )
    ...
    final_rows = tuple(
        replace(row, rank=ranks.get(row.symbol))
        for row in sorted(rows, key=lambda row: row.symbol)
    )
    eligible = tuple(
        sorted(
            (row for row in final_rows if row.rank is not None),
            key=lambda row: row.rank,
        )
    )
    snapshot = HistoricalUniverseSnapshot(decision, final_rows)
    return HistoricalDiscoveryResult(
        decision, snapshot, eligible, tuple(sources), config
    )
