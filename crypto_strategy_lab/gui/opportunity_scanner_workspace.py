...
class OpportunityScannerWorkspace(QWidget):
    ...
    def __init__(self, service, parent=None):
        ...
        config = QGroupBox("Scan configuration"); form = QFormLayout(config)
    ...
    def _mode_changed(self):
        ...
        self.execution.setVisible(historical); self.decision_time.setVisible(historical and not ranged)
        for widget in (self.range_start,self.range_end,self.cadence,self.planned): widget.setVisible(ranged)
    ...
    @staticmethod
    def _utc(edit): return edit.dateTime().toPython().replace(tzinfo=timezone.utc)
    ...
    def _progress(self,event):
        ...
        self.progress_text.setText(f"Stage: {event.stage_index}/{event.stage_count} — {event.message}{decision}\nElapsed: {self._duration(event.elapsed_seconds)}{average}{eta}")
