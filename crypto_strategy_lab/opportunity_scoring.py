...
LEGACY_VOLATILITY_V1 = OpportunityScoringModelDefinition("legacy_volatility", "1", tuple(OpportunityComponent(*x) for x in (
    ("realized_volatility", .30), ("atr_pct", .25), ("recent_range_pct", .20), ("range_expansion", .10), ("volume_ratio", .15))), True)
...
class OpportunityScoreRow:
    symbol: str; discovery_rank: int; decision_time: datetime; strategy_interval: str
    feature_timestamp: datetime; available_at: datetime; raw_components: Mapping[str, float]
...
def _row(s,m,match,norm,rank,missing):
    ...
    raw={} if match is None else {c.name:float(match[0][c.name]) for c in m.components}
...
class HistoricalOpportunityEvaluator:
    ...
    def evaluate(self,row: OpportunityScoreRow, candles: pd.DataFrame, decision_close: float) -> OpportunityOutcome | None:
        ...
        times=pd.to_datetime(candles.period_start,utc=True); future=candles[(times>start)&(times<=start+self.horizon)]
        if future.empty or not row.atr or row.atr<=0:return None
...
def build_comparison(scores: Sequence[OpportunityScoreRow], outcomes: Mapping[tuple[str, datetime], OpportunityOutcome],
    ...
    for row in unique.values():
        controls.append(("discovery_order",row.discovery_rank,row))
        controls.append(("no_second_stage",row.discovery_rank,row))
    modeled=[(f"{r.model_name}_v{r.model_version}",r.model_rank,r) for r in scores if r.status==ScoreStatus.SCORABLE]
    result=[]
    for model in ("discovery_order","legacy_volatility_v1","balanced_activity_v1","momentum_activity_v1","no_second_stage"):
        pool=[x for x in controls+modeled if x[0]==model and x[1] is not None]
        decisions=sorted({x[2].decision_time for x in pool})
        ks=(None,) if model=="no_second_stage" else tuple(top_ks)
        for k in ks:
            selected=[]
            for decision in decisions:
                day=sorted((x for x in pool if x[2].decision_time==decision),key=lambda x:(x[1],x[2].discovery_rank,x[2].symbol))
                selected.extend(day if k is None else day[:k])
            evaluated=[(row,outcomes.get((row.symbol,row.decision_time))) for _,_,row in selected]
            evaluated=[x for x in evaluated if x[1] is not None]
            result.extend(_aggregate(model,"ALL","ALL",k,decisions,pool,selected,evaluated))
            years=sorted({r.decision_time.year for r,_ in evaluated})
            regimes=sorted({r.market_regime for r,_ in evaluated if r.market_regime})
            for year in years: result.extend(_aggregate(model,"YEAR",str(year),k,decisions,pool,selected,[(r,o) for r,o in evaluated if r.decision_time.year==year]))
            for regime in regimes: result.extend(_aggregate(model,"REGIME",regime,k,decisions,pool,selected,[(r,o) for r,o in evaluated if r.market_regime==regime]))
    ...
    for row in result:
        ...
        row["lift_vs_discovery_order_forward_range_pct"] = None if not base or not base["mean_forward_range_pct"] else row["mean_forward_range_pct"]/base["mean_forward_range_pct"]-1
    return tuple(result)


def _aggregate(model,stratum_type,stratum,k,decisions,pool,selected,evaluated):
    ...
    acquired=len({(x[2].symbol,x[2].decision_time) for x in pool})
    return [{"model":model,"top_k":"ALL" if k is None else k,"stratum_type":stratum_type,"stratum":stratum,
        "decision_count":len(decisions),"candidate_count":len(selected),"scorable_count":len(evaluated),
        ...
        "coverage":len(evaluated)/len(selected) if selected else 0.0,"unscorable_rate":1-len(selected)/acquired if acquired else 1.0}]
