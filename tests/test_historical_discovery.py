...
def candle(symbol, start, *, available_at=None, open="100", high="120", low="90", close="110", volume="20000000"):
    end = start + timedelta(days=1)
    return {"symbol": symbol, "period_start": start, "period_end": end,
            "available_at": available_at or end, "open": open, "high": high,
            "low": low, "close": close, "quote_volume": volume}
...
def test_future_candles_cannot_change_selection_or_ranks():
    ...
    rows = {"AAAUSDT": [candle("AAAUSDT", prior - timedelta(days=1))],
            "BBBUSDT": [candle("BBBUSDT", prior - timedelta(days=1), high="115")]}
    before = discover_historical_universe(Store(rows), rows, T)
    rows["AAAUSDT"].append(candle("AAAUSDT", prior, available_at=T + timedelta(seconds=1), high="9999"))
    ...
    assert [(r.symbol, r.period_start, r.range_percent) for r in before.snapshot.rows] == [
...
def test_available_at_boundary_is_allowed_and_later_is_not():
    boundary = candle("AAAUSDT", T - timedelta(days=1), available_at=T, high="130")
    later = candle("AAAUSDT", T - timedelta(days=2), available_at=T + timedelta(microseconds=1), high="999")
    result = discover_historical_universe(Store({"AAAUSDT": [later, boundary]}), ["AAAUSDT"], T)
...
def test_deterministic_under_symbol_and_row_shuffle():
    ...
    rows = {symbol: [candle(symbol, T - timedelta(days=3)),
                     candle(symbol, T - timedelta(days=2))] for symbol in symbols}
...
def test_utc_equivalence_and_naive_rejection():
    ...
    assert discover_historical_universe(Store(rows), rows, T) == discover_historical_universe(Store(rows), rows, equivalent)
...
def test_source_signature_is_canonical_and_partition_sensitive():
    rows = {"AAAUSDT": [candle("AAAUSDT", T - timedelta(days=2))]}
    first = discover_historical_universe(Store(rows, {"AAAUSDT": ["p1"]}), rows, T)
    repeated = discover_historical_universe(Store(rows, {"AAAUSDT": ["p1"]}), rows, T)
    changed = discover_historical_universe(Store(rows, {"AAAUSDT": ["p2"]}), rows, T)
    assert first.sources == repeated.sources
    assert first.sources[0].signature.cache_identity() != changed.sources[0].signature.cache_identity()
    assert first.snapshot.rows[0].source_identity != changed.snapshot.rows[0].source_identity
...
def test_missing_invalid_and_low_volume_are_auditable_and_fail_closed():
    ...
    result = discover_historical_universe(Store({"BADUSDT": [invalid], "LOWUSDT": [low], "NONEUSDT": []}),
                                          ["NONEUSDT", "LOWUSDT", "BADUSDT"], T)
    states = {row.symbol: row.rejection_reasons for row in result.snapshot.rows}
    assert states == {"BADUSDT": ("invalid_or_missing_numeric_value",),
...
def test_historical_path_uses_only_data_lake_contract_not_live_client(monkeypatch):
    ...
    monkeypatch.setattr("crypto_strategy_lab.data.binance.universe.BinanceUsdMDiscoveryClient._get", forbidden)
    store = Store({"AAAUSDT": [candle("AAAUSDT", T - timedelta(days=2))]})
    result = discover_historical_universe(store, ["AAAUSDT"], T,
        HistoricalDiscoveryConfig(minimum_quote_volume=Decimal("0")))
    assert ranks(result) == [("AAAUSDT", 1)]
    assert {call[0] for call in store.calls} == {"source_signature", "load_dataset"}
